...
public class SparkBenchmark {
  ...
  public static class InputSource {
    ...
    public void setup() {
      ...
      try {
        ...
        schema = (StructType) Utilities.convertToSparkType(orcSchema);
      } catch (IOException e) {
  ...
  @Benchmark
  public void fullRead(InputSource source, ExtraCounters counters) {
  ...
  @Benchmark
  public void partialRead(InputSource source, ExtraCounters counters) {
    FileSystem.Statistics statistics = source.fs.getLocalStatistics();
    statistics.reset();
    List<Filter> filters = new ArrayList<>();
    List<Tuple2<String,String>> options = new ArrayList<>();
    ...
    Function1<PartitionedFile,Iterator<InternalRow>> factory =
        source.formatObject.buildReaderWithPartitionValues(source.session,
            ...
            (StructType) Utilities.convertToSparkType(readSchema),
            JavaConverters.collectionAsScalaIterableConverter(filters).asScala().toSeq(),
            (Map<String, String>)Map$.MODULE$.apply(optionsScala), source.conf);
    PartitionedFile file = new PartitionedFile(InternalRow.empty(),
        source.path.toString(), 0, Long.MAX_VALUE, new String[0]);
  ...
  @Benchmark
  public void pushDown(InputSource source, ExtraCounters counters) {
    FileSystem.Statistics statistics = source.fs.getLocalStatistics();
    statistics.reset();
    List<Filter> filters = new ArrayList<>();
    switch (source.dataset) {
